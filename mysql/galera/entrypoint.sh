...
# installs mysql.user and the initial tables required to bootstrap mysql.
# mysql_install_db
# mysqld
#!/bin/bash
...
# if NUM_NODES not passed, default to 2
if [ -z "$NUM_NODES" ]; then
  NUM_NODES=2
fi
...
if [ -n "$GALERA_CLUSTER" ]; then
  ...
  for NUM in `seq 1 $NUM_NODES`; do
    if [ $(expr "$HOSTNAME" : "shard${NUM}") -eq 0 ]; then
      if [ $WSREP_CLUSTER_ADDRESS != "gcomm://" ]; then
        WSREP_CLUSTER_ADDRESS="${WSREP_CLUSTER_ADDRESS},"
      fi
      WSREP_CLUSTER_ADDRESS="gcomm://shard${NUM}.default.svc.cluster.local"
      echo "appending shard ${WSREP_CLUSTER_ADDRESS}"
    else
      echo my hostname: $HOSTNAME, ignoring
    fi
  done
